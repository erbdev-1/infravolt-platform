...
describe("PUBLIC_ROUTE_PATHS", () => {
  it("contains only bounded canonical pathnames", () => {
    expect(PUBLIC_ROUTE_PATHS).toHaveLength(151);
  ...
  it("includes the Phase 2 Data Centre Infrastructure Hub exactly once", () => {
    const matches = PUBLIC_ROUTE_PATHS.filter((pathname) => pathname === "/data-centres");
    expect(matches).toHaveLength(1);
  });
});
