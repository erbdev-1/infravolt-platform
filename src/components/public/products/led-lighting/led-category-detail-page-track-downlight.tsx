...
import {
  TRACK_DOWNLIGHT_HERO_BACKGROUND,
  TRACK_DOWNLIGHT_HERO_BACKGROUND_ALT,
  TRACK_DOWNLIGHT_HERO_FOREGROUND,
  TRACK_DOWNLIGHT_HERO_FOREGROUND_ALT,
  TRACK_DOWNLIGHT_SUPPORT_CTA_IMAGE,
...
export function LedCategoryDetailPageTrackDownlight({
  market,
  pageContent,
  backHref = "/products/led-systems",
  parentBreadcrumb,
  heroBackgroundImage = TRACK_DOWNLIGHT_HERO_BACKGROUND,
  heroBackgroundImageAlt = TRACK_DOWNLIGHT_HERO_BACKGROUND_ALT,
  heroForegroundImage = TRACK_DOWNLIGHT_HERO_FOREGROUND,
  heroForegroundImageAlt = TRACK_DOWNLIGHT_HERO_FOREGROUND_ALT,
  supportCtaImage = TRACK_DOWNLIGHT_SUPPORT_CTA_IMAGE,
  supportCtaImageAlt = TRACK_DOWNLIGHT_SUPPORT_CTA_IMAGE_ALT,
  supportRequestHref = SUPPORT_REQUEST_HREF,
}: Readonly<{
  market: MarketCode;
  pageContent?: LedCategoryDetailContent;
  backHref?: string;
  parentBreadcrumb?: Readonly<{ href: string; label: string }>;
  heroBackgroundImage?: string;
  heroBackgroundImageAlt?: string;
  heroForegroundImage?: string;
  heroForegroundImageAlt?: string;
  supportCtaImage?: string;
  supportCtaImageAlt?: string;
  supportRequestHref?: string;
}>) {
  const content = pageContent ?? trackDownlightContentForMarket(market);
      ...
      <section className={styles.series}>
        ...
        {content.seriesGroups?.map((group) => (
          <div className={styles.seriesGroup} key={group.heading}>
            <div className={styles.seriesGroupHeader}>
              <h3 className={styles.seriesGroupHeading}>{group.heading}</h3>
              {group.href && group.actionLabel ? (
                <Link className={styles.seriesGroupLink} href={group.href}>
                  {group.actionLabel}<span aria-hidden="true">→</span>
                </Link>
              ) : null}
            </div>
