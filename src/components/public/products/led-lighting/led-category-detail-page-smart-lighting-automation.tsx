...
import {
  SMART_LIGHTING_HERO_BACKGROUND,
  SMART_LIGHTING_HERO_BACKGROUND_ALT,
  SMART_LIGHTING_HERO_FOREGROUND,
  SMART_LIGHTING_HERO_FOREGROUND_ALT,
  SMART_LIGHTING_SUPPORT_CTA_IMAGE,
  SMART_LIGHTING_SUPPORT_CTA_IMAGE_ALT,
  smartLightingAutomationContentForMarket,
} from "@/data/products/led-lighting/smart-lighting-automation";
...
// Dedicated category component for Smart Lighting & Automation — mirrors
// led-category-detail-page.tsx's structure exactly (same shared
...
const APPLICATION_ICONS = [IconFactory, IconBusbarConnect, IconAutomation, IconSmartEnvironment] as const;
...
// data only exercises "powerline-comms" / "mounting-options" /
// "applications" / "camera" / "control" / "protection", same as
...
const SUPPORT_REQUEST_HREF = "/uk-support?request=technical-pack&product=smart-lighting-automation";

export function LedCategoryDetailPageSmartLightingAutomation({
  market,
}: Readonly<{
  market: MarketCode;
}>) {
  const content = smartLightingAutomationContentForMarket(market);

  return (
    <main className={styles.page}>
      <section className={styles.hero}>
        <Image
          alt={SMART_LIGHTING_HERO_BACKGROUND_ALT}
          className={styles.heroBackground}
          fill
          priority
          sizes="100vw"
          src={SMART_LIGHTING_HERO_BACKGROUND}
        />
        <div className={styles.heroScrim} />

        <div className={styles.heroForeground}>
          <Image
            alt={SMART_LIGHTING_HERO_FOREGROUND_ALT}
            fill
            priority
            sizes="(min-width: 1100px) 46vw, 90vw"
            src={SMART_LIGHTING_HERO_FOREGROUND}
      ...
      <section className={styles.supportCta}>
        <Image
          alt={SMART_LIGHTING_SUPPORT_CTA_IMAGE_ALT}
          className={styles.supportCtaImage}
          fill
          sizes="100vw"
          src={SMART_LIGHTING_SUPPORT_CTA_IMAGE}
        />
