...
import {
  OUTDOOR_INFRASTRUCTURE_HERO_BACKGROUND,
  OUTDOOR_INFRASTRUCTURE_HERO_BACKGROUND_ALT,
  OUTDOOR_INFRASTRUCTURE_HERO_FOREGROUND,
  OUTDOOR_INFRASTRUCTURE_HERO_FOREGROUND_ALT,
  OUTDOOR_INFRASTRUCTURE_SUPPORT_CTA_IMAGE,
  OUTDOOR_INFRASTRUCTURE_SUPPORT_CTA_IMAGE_ALT,
  outdoorInfrastructureLightingContentForMarket,
} from "@/data/products/led-lighting/outdoor-infrastructure-lighting";
...
// Dedicated category component for Outdoor & Infrastructure Lighting.
// It preserves the established Industrial & High-Bay structure and
// shared visual system while reading this category's own data and assets.
const APPLICATION_ICONS = [IconStreetLight, IconTunnel, IconFactory, IconSurfaceMount, IconApplications] as const;
...
// data only exercises "rugged-body" / "protection" / "applications",
// same as led-category-detail-page.tsx's own FEATURE_ICONS. "camera" /
// "powerline-comms" were added for the Smart Lighting & Automation
// category's series cards — never triggered by this category's data.
const FEATURE_ICONS = {
...
const SUPPORT_REQUEST_HREF = "/uk-support?request=technical-pack&product=outdoor-infrastructure-lighting";

export function LedCategoryDetailPageOutdoorInfrastructure({
  market,
}: Readonly<{
  market: MarketCode;
}>) {
  const content = outdoorInfrastructureLightingContentForMarket(market);

  return (
    <main className={styles.page}>
      <section className={styles.hero}>
        <Image
          alt={OUTDOOR_INFRASTRUCTURE_HERO_BACKGROUND_ALT}
          className={styles.heroBackground}
          fill
          priority
          sizes="100vw"
          src={OUTDOOR_INFRASTRUCTURE_HERO_BACKGROUND}
        />
        <div className={styles.heroScrim} />

        <div className={styles.heroForeground}>
          <Image
            alt={OUTDOOR_INFRASTRUCTURE_HERO_FOREGROUND_ALT}
            fill
            priority
            sizes="(min-width: 1100px) 46vw, 90vw"
            src={OUTDOOR_INFRASTRUCTURE_HERO_FOREGROUND}
              ...
              <article className={styles.applicationCardDetailed} key={application.title}>
                ...
                <div className={styles.applicationDetailBody}>
                  <div className={styles.applicationDetailHeading}>
                    {ApplicationIcon ? <ApplicationIcon className={styles.applicationDetailIcon} /> : null}
      ...
      <section className={styles.supportCta}>
        <Image
          alt={OUTDOOR_INFRASTRUCTURE_SUPPORT_CTA_IMAGE_ALT}
          className={styles.supportCtaImage}
          fill
          sizes="100vw"
          src={OUTDOOR_INFRASTRUCTURE_SUPPORT_CTA_IMAGE}
        />
